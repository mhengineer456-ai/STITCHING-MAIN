...
const DoriManagement = ({ user, onNavigate }) => {
  ...
  const filterOptions = useMemo(() => {
    ...
    // Extract all unique dori placements
    const allPlacements = data.flatMap(row => {
      const placements = safeJSONParse(row['Selected Placements'], []);
      return placements;
    }).filter(Boolean);
    
    const doriPlacements = [...new Set(allPlacements)];
    
    return { garmentTypes, supervisors, doriPlacements };
  }, [data]);
  ...
  const filteredData = useMemo(() => {
    ...
    // Add dori placement filter
    if (filters.doriPlacement) {
      result = result.filter(row => {
        const placements = safeJSONParse(row['Selected Placements'], []);
        return placements.some(placement => 
          placement.toLowerCase().includes(filters.doriPlacement.toLowerCase())
        );
      });
    }
    ...
    autoTable(doc, {
      ...
      didParseCell: function(data) {
        ...
        if (data.section === 'body') {
          ...
          const doriStatus = rowData[10]; // Status is now at index 10
          
          if (doriStatus === 'PENDING') {
            // Highlight entire row for pending dori
        ...
        <div style={styles.headerLeft}>
          ...
          <div style={styles.headerContent}>
            ...
            <p style={styles.subtitle}>
              {user ? `Viewing orders for ${user.name}` : 'Manage and track all dori material requirements'}
            </p>
          </div>
        </div>
      ...
      <div style={styles.controlsSection}>
        ...
        <div style={styles.filtersGrid}>
          ...
          <div style={styles.filterGroup}>
            ...
            <select
              value={filters.doriPlacement}
              onChange={(e) => handleFilterChange('doriPlacement', e.target.value)}
              style={styles.filterSelect}
            >
              <option value="">All Placements</option>
              {filterOptions.doriPlacements.map(placement => (
